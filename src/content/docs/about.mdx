---
title: Advanced Research Computing for Computational Fluid Mechanics
description: Outline of concepts covered in section 1.
tableOfContents: false
---

<p align="center">
<svg color='var(--sl-color-text-accent)' overflow='visible' viewbox='30 30 1024 1024' width='20%' height='auto' fill='none' stroke='var(--sl-color-text-accent)' stroke-width='52' opacity='1' stroke-linecap='round' stroke-linejoin='round' font-size='1em'><g><path d="M139.142 283.654C139.142 202.604 204.846 136.9 285.896 136.9L742.14 136.9C823.19 136.9 888.894 202.604 888.894 283.654L888.894 739.898C888.894 820.947 823.19 886.651 742.14 886.651L285.896 886.651C204.846 886.651 139.142 820.947 139.142 739.898L139.142 283.654Z"/><path d="M237.271 700.043C229.787 692.637 217.09 698.069 217.278 708.596L218.64 784.843C218.755 791.292 224.052 796.44 230.502 796.371L305.387 795.569C315.814 795.457 320.93 782.821 313.518 775.487L237.271 700.043Z" fill="currentColor" stroke="none"/><path d="M115.178 382.082L30.7625 382.103"/><path d="M994.98 379.767L910.565 379.788"/><path d="M648.316 113.056L648.541 28.641"/><path d="M646.106 996.981L645.925 912.566"/><path d="M113.435 511.754L29.0199 511.776"/><path d="M993.238 509.439L908.822 509.461"/><path d="M518.649 110.934L518.874 26.5193"/><path d="M516.431 995.484L516.25 911.069"/><path d="M115.178 646.078L30.7625 646.099"/><path d="M994.98 643.763L910.565 643.784"/><path d="M384.321 112.284L384.546 27.8692"/><path d="M382.111 997.481L381.93 913.066"/><path d="M227.041 294.661C227.041 294.661 329.102 298.693 429.094 345.016C529.086 391.338 545.237 440.185 627.009 479.951C708.78 519.717 800.995 510.956 800.995 510.956"/><path d="M227.041 509.439C227.041 509.439 333.509 509.439 412.85 537.621C505.166 570.411 548.113 593.164 645.925 607.993C727.659 620.385 800.995 619.42 800.995 619.42"/></g></svg></p>


# About ARC4CFD
Computational Fluid Dynamics (CFD) is a field of computational physics that has a very high utilization of modern advanced research computing (ARC) resources. CFD codes solve the discretized form of the governing equations for fluid motion, which is a high degree of freedom mathematical problem. The spatial and temporal resolution required to solve these problems makes CFD particularly well suited to take advantage of the full benefits of large-scale, parallelized high-performance computing on ARC infrastructure.  This course is designed to help students with a cursury knowledge of fluid dynamics and CFD to bridge the gap to effectively leverage modern ARC resources. 


# Teaching philosophy
We developed as an asynchronous online course to accompany learner towards gaining an understanding of the usage of CFD on modern ARC resources. The course is divided into three sections, which are made up of individual classes each with clearly defined learning outcomes. The multimodal course content includes text, videos, interactive quizzes, and hands-on examples coveing various aspects of ARC. In Section 2, the example of a backward facing step is woven through all the classes and represents a comprehensive workflow for the usage of CFD on HPC. The course combines both technical knowledge with practical knowledge, with the aim of 


# Learning outcomes of the course
At the end of this course, the learner should be able to:
1- **Assess the parameters influencing computational cost and their modelling assumptions**
2- **Conduct an a priori estimate of the computational cost of a CFD simulation**
3- **Determine the most efficient workflow and strategies for CFD on HPC systems**
4- **Find the optimal parameters and settings to run a CFD simulation on HPC**




# Target audience
The target audience for this course is:
- new graduate students in computational physics or engineering, 
- experimentalist and theoreticians complementing their work with numerical simulations,
- undergraduate students on student design teams interested in leveraging CFD with HPC.

Experienced researchers in CFD but without a strong knowledge in high-performance computing may want to jump directly to sections 2.3, 2.5, and 3.2.






This course was developed in the [Multi-Physics Interaction Lab](mpilab.ca) at the University of Waterloo, Canada with the financial support of Compute Ontario.


 ![HPCcompromise.](../../assets/mpilab_wide.png "Estimate HPC costs of the CFD simulation") | ![HPCcompromise.](../../assets/UniversityOfWaterloo_logo.png "Estimate HPC costs of the CFD simulation") |  ![HPCcompromise.](../../assets/ComputeOntarioLogo.png "Estimate HPC costs of the CFD simulation") 
 :-------------------------:|:-------------------------:|:-------------------------:
[**MPI**LAB](www.mpilab.ca)             |  [University of Waterloo](www.uwaterloo.ca) |  [Compute Ontario](https://www.computeontario.ca)

