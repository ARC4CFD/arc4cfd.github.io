...
## Transform PDEs to algebraic ODEs
...
In the following, we provide a parallelized code, written in Python programming language, to solve the 2D Poisson equation as described above. In the code, the user specifies the following input parameters:
...
```bash
[username@laptop ~]$ sftp username@narval.computecanada.ca
```
...
<Tabs group="tab-group">
    <TabItem label="1 CPU">
    ```bash
    [username@nc30301 ~]$ mpiexec -n 1 python heat_conduction.py
    ...
    ```bash
    [username@nc30301 ~]$ mpiexec -n 4 python heat_conduction.py
    ...
    ```bash
    [username@nc30301 ~]$ mpiexec -n 8 python heat_conduction.py
    ...
    ```bash
    [username@nc30301 ~]$ mpiexec -n 16 python heat_conduction.py
    ...
    ```bash
    [username@nc30301 ~]$ mpiexec -n 32 python heat_conduction.py
