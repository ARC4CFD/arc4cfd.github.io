...
Where $T^{k}$ is the approximate solution at the $k$-th iteration. To have a measure of the residual comparable at different mesh resolutions, the RMS is often used as a more accurate parameter $||r^{k}||/\sqrt{n}$.
...
If you plan to run anything more than a trivially small case, you should request an interactive node. You are now ready to test your code on the cluster. The code can be executed using the following script:
```bash
...
Once downloaded on the local machine, results can be visualized using the software of preference. In the following we make the example of a simple Matlab routine to plot the temperature contours:
