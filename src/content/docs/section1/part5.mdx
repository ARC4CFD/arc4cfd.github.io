...
This is a crucial step towards the numerical solution, and many problems in CFD or other fields arise, often, due to a poor application of boundary conditions. In this very simple case, the temperature is set at every face of the domain. Given that the plate does not exist anymore as we have substituted it with a set of **nodes**, the boundary condition must be set on the nodes that are sitting on the boundary. Following the schematic figure above, one might say:
