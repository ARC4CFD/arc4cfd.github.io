...
This course uses open-source CFD tools on modern high-performance computing systems. The course was  specifically developed for use on the Digital Research Alliance of Canada HPC clusters, although users of other similar high-performance clusters (e.g. linux-based systems, using SLURM workload manager and module system) can use the course with minimal changes. Nonetheless, access to an HPC cluster is not essential for completing some parts of the course. The students can install the open-source software and run the test cases on their local computer; the only requirement is to use a multi-core processor. There will be slight differences in installation procedures, specific command lines and visualization. Users on other HPC clusters are invited to reach out to their local help desk for assistance.
...
- **GMsh** (version above 4.XX ) The source code can be [downloaded](https://gmsh.info) and installed, binary releases of  4.13.1 require Windows ≥ 10, Linux with glibc ≥ 2.24, macOS (x86 - Intel processors) ≥ 10.15 or macOS (ARM - Apple M-series processors) ≥ 12
