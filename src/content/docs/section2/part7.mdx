...
After completing [section 2.6](https://arc4cfd.github.io/section2/part6/), our large-scale CFD simulation is now running on the HPC system, and printing results as specified by the user. We also learned how to monitor the simulation using simple line commands on terminal, and the very useful plotting tool [Gnuplot](http://www.gnuplot.info/). At this point **the fun part** begins as we can dive into the visualization of the results. Ultimately, a CFD simulation is nothing but a **numerical experiment** out of which we get field data (loads of it), on which we will perform our analysis, and **maybe** discover something `NEW`. The purpose of this section is therefore to propose a consistent approach towards data analysis in CFD, and to present the available options in terms of flow visualization on HPC.  
