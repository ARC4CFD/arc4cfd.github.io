...
It's very important to remember that 1 in 12 men in the world are affected by color blindness. 
:::
...
2. Lines (representing different simulations) are all of the same style (i.e. solid line). Bad for color blind people.
...
2. Lines (representing different simulations) use **different styles** and **different colors**. Very good for color blind people.
...
Before diving too much into endless visualization sessions of our fresh CFD data, we should **always** answer the question: **can I trust my results**? This is a crucial question to answer as the final goal of any research problem is, most likely, scientific publication. In order to answer this **crucial** question, we suggest following a rigorous process composed of **TWO** steps:
...
In the figure above we notice that the **dash** and **solid** curves at all locations (differenct colors) are very close to each other. One can estimate the error between the two profiles at each location, and if the difference between the profiles is less than 5% we can conclude that **grid convergence** has been reached on the medium-fine grid of about 400000 points. Any simulation on a grid finer than this will not consitute a significant improvement on the solution and will therefore be a **waste** of computational resources.
