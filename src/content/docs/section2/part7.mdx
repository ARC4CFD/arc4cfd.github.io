...
After which the preferred software will pop up on the screen, and the user can star the visualization session as usually done on the local machine.
...
:::note[Beware]
When using interactive **remote-desktop** visualization, both the datasets and the rendering ARE ON the cluster. You will probably have solved memory issues since the VDI nodes have a much larger mmemory compared to your local machine, however you might experience some LAG during rendering as the **screenshots** from the VNC Server are trasmitted **across the network** to the VNC Viewer. 
:::

This small issue can be solved by ...

### Interactive client-server visualization [TO DO]
...
## Grid convergence and code validation [TO DO]
