...
- `/home` has a capacity of small which  is suitable for code development, source code, small parameter files, job submission scripts and version control. Note that the home drive cannot be written to from the compute nodes.
...
As we have space and plan to run the simulations from `/scratch`, if it's not already done, we can clone from the course [GitHub repository](https://github.com/ARC4CFD/arc4cfd/tree/master). 
