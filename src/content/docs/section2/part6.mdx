...
1. **Mesh size**: the first and most important choice is the **number of points**. We prepared the case so that students can run several **meshes**. Here, however, for semplicity's sake, we only follow the example of the coarsest mesh of $237000$ grid points (named `bfs_200k_DDES` in OpenFoam, and `Coarse` in SU2 examples).
...
<Tabs group="tab-group">
    ...
    <Caption>Root-mean-square $rms$ of the three velocity components, $U_x$, $U_y$, and $U_z$.</Caption>
