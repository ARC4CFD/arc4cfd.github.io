...
- `/home` has a small capacity which  is suitable for code development, source code, small parameter files, job submission scripts and version control. Note that we cannot write to the  `/home` drive from the compute nodes.
- `/project (group rrg-piname-ac)` is a directory that is linked to your principal investigator's account and meant for longer-term storage and  sharing data among members of a research group. 
...
As we have space on the `/home` drive, if it's not already done, we can clone from the course [GitHub repository](https://github.com/ARC4CFD/arc4cfd/tree/master). The repository contains the input files and mesh for the examples (and is of modest size), therefore the `/home` drive will allow us to modify and save the examples prior to copying the cases to `/scratch` for running the simulations.
...
Best practices in folder naming conventions:
 ...
 - **Write down the naming convention**: Write down the naming convention in the data management plan (DMP), more in [Section 3](https://arc4cfd.github.io/section3/outline/), or in a README file.
...
2. **Numerics**: the selection of the numerical details for both the temporal and spatial calculations will directly impact the computational cost. The specific details on the selection of the numerics falls outside the scope of ARC4CFD, therefore it is only a list of some of the considerations to set: 
...
<p style={{ color: "purple"}}>  Run the numerical simulation of the same backward facing step flow for the mesh containing $\approx 400000$ grid points. </p>

<p style={{ color: "purple"}}> **2.6.1 What would be the time step size $\Delta t$ required by the CFL stability condition?**</p>
<MultipleChoice>
    <Option>
      The same
    </Option>
...
<p style={{ color: "purple"}}> **2.6.2 What would be the writeInterval required to still print results every 2 milliseconds?**</p>
<MultipleChoice>
    <Option>
      20
    </Option>
...
<Tabs group="tab-group">
    <TabItem label="OpenFOAM">
        ...
        ![Residuals.](../../../assets/figs_section2/residual-plot-of.jpeg "Residuals.")
...
Many HPC systems use fair share schedulers which often impose maximum time durations for each submitted job. On most Digital Research Alliance clusters, [the maximum walltime of a job is 24 hours](https://docs.alliancecan.ca/wiki/Running_jobs). Therefore, restart files (also called breakpoint) need to be output in order to run the simulation for more than 24 hours.  To optimize the computational usage, we should seek to have a restart file written immediately before the end of the simulation.

![Grid convergence.](../../../assets/figs_section2/ARC4CFD_restartTiming.png "Optimized Restarts")
        <Caption>With a fixed timestep/iteration restart output, there will be poorly utilized resources (top). Optimal usage will align the writing of the restart file with the wallclock time of the simulation.</Caption>
...
This is an external test where the goal is to assess the the models are correctly implemented (verification) and the accuracy of the models to represent real-world flow (validation). The verification and validation processes are [well established and standardized among the CFD community](https://www.grc.nasa.gov/www/wind/valid/tutorial/overview.html). We define:
...
For well-established codes, verification of the implemented models is usually done prior to the codes release (see e.g. [openFoam's V\&V](https://www.openfoam.com/documentation/guides/latest/doc/guide-verification-validation.html)). Validation, on the other hand, is a critical step to assure the validity and gain confidence in the CFD results. As CFD is predictive tool, it is often difficult to validate the specific test case. In those cases, there are many **canonical flows** whose sole purpose is to serve as benchmark (lead driven cavity flow, backward facing step, turbulent boundary layer, periodic channel flow, etc.).
