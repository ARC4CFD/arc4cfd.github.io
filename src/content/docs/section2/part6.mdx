...
<Tabs group="tab-group">
    ...
    ```bash
    [username@gra-login1 ~]$ cp -r * ./scratch/01_BFS_openFOAM
    ```
    </TabItem>
    <TabItem label="/scratch/02_BFS_SU2">
    ```bash
    [username@gra-login1 ~]$ cp -r ./Coarse/ ./scratch/02_BFS_SU2
    ```
    </TabItem>
</Tabs>
</Box>
...
<Tabs group="tab-group">
    ...
    ```bash
    [username@gra796 ~]$ cp -r * ./case/* ./bfs_200k_DDES
    ```
    3. **Generate mesh** from file using the `gmsh` utility:
    ```bash
    [username@gra796 ~]$ gmsh mesh/bfs_200k.geo -3 -o mesh/bfs_200k.msh -format msh2
    ```
    ...
    ```bash
    [username@gra796 ~]$ cd ./bfs_200k_DDES
    [username@gra796 ~]$ ./Allrun
    ```
    ...
    ```bash
    [username@gra796 ~]$ gmsh Backstep_str_mesh.geo -0
    ```
    3. **Start the simulation**:
    ```bash
    [username@gra796 ~]$ mpirun -n 64 SU2_CFD Backstep_str_config.cfg
    ```    
    </TabItem>
</Tabs>
</Box>
...
<Tabs group="tab-group">
    <TabItem label="OpenFOAM steps">
    ```bash
    [username@gra-login1 ~]$ sbatch run_jobscript.sh
    Submitted batch job 26236582 
    ```
    </TabItem>
    <TabItem label="SU2 steps">
    ```bash
    [username@gra-login1 ~]$ sbatch su2job_StdEnv.sh
    Submitted batch job 26236582 
    ``` 
    </TabItem>
</Tabs>
...
<Tabs group="tab-group">
    <TabItem label="OpenFOAM">
        ...
        <details>
            ...
            ```bash
            [username@gra-login1 ~]$ vim log.pimpleFoam
            Starting time loop
        ...
        ```bash
        [username@gra-login1 ~]$ module load gnuplot 
        ...
        ```bash
        [username@gra-login1 ~]$ gnuplot plot-residuals
        ```
