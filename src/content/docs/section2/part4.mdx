...
The pre-processing step is not specific to CFD workflow on HPC systems. In fact, many of these steps such as geometry generation, mesh creation, and input file setup are integral steps to any CFD simulation, although there are specific considerations to the HPC that will be discussed herein.
...
1. Construct the geometry (either in dedicated CAD software or, if simple enough, within meshing tools)
