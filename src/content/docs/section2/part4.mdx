...
$$ Aspect Ratio = \frac{max(x_1, x_2, x_3,...x_n)}{min(x_1, x_2, x_3,...x_n)} $$
 This formula is used for hexaheadral meshes, slight a modification of  this formula (using an internal sphere) is used for tetrahedral or other cell geometries.  This computation is conducted in each cell of the domain and the maximum value is presented
![CFD Example.](../../../assets/figs_section2/ARC4CFD_aspectratio.png "Example CFD")

:::tip[Rule of thumb]
For wall-resolved simulations, the aspect ratio will be large. Based on the discussion in the previous class, we said that $dy=y^+ and dx\approx 100 y^+$, for RANS calculations.
:::
...
- **Volume ratio**: This metric reveals the largest volumes between neighboring cells. Strong discrepencies in cell size can result in large numerical error and thus instability of the simulation. The sensitivity of this parameter depends on a number of factors such as order of the numerical method. If this value is too large, a smoother transition should be considered.

Normally, for a structured mesh.
