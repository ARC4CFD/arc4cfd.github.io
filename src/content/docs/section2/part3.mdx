...
$$dx \approx 20*2.42\times 10^{-4} \approx  0.005$$
...
Due to the turbulent mixing near the wall and the non-slip condition at the wall, the larger velocity gradient of the turbulent boundary layer will result in a smaller grid spacing than a typical laminar boundary layer of the same height. To assess the mininal grid spacing, we recall the typical characteristics of the inner layer of a turbulent boundary layer.  To do so, we can non-dimensionalize the wall distance $y$ with the viscosity and friction velocity such that we can define: 
