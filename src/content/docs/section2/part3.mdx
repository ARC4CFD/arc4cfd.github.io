...
Time advancement of the CFD problem has a significant impact on the total HPC costs.  Starting from the intial condition (for an  unsteady problem), the governing equations need be advanced by a time step, $dt$, that may be constant or varying. Naturally, the allowable $dt$ will constrained by  either:

- stability of the temporal scheme  (numerics);
- characteristic time of the smallest resolved timescale (physics). 

The stricter of the two time constraints will impose a limit on $dt$. Each of timestep constraits is discussed below.
 ...
 - **Implicit time advancement**:  unconditionally (well most of the time) stable
...
```bash ins=/Memory.*/
    [username@gra-login1 ~]$ seff  [job ID]
...
To estimate the total HPC cost, we need to know the approximate physical time per-time-step, $dt$, for the simulation: $dt_{wall}$. This time step $dt_{wall}$ naturally depends on the number of processors, therefore a reasonable estimate should be made by the user.

Let's say, we plan to run a simulation with $N_{total}= 1\times10^6$ grid points, over $N_{time} 250,000$ time steps, which requires a total memory of $\mathcal{M}_{total} = 250 Gb$. The first important consideration is that the memory needed will dictate the minimal number of processors needed. For this case, on Niagara, we would need *at least* 2 cores (80 processors). The second step is to determine the approximate physical time needed for each timestep. Based on previous usage, you can estimate this problem to take $dt_{wall}=3$ s  on 80 processors. Therefore, we can anticipate the total HPC time to be:
...
The storage requirements are easy to by typing:
