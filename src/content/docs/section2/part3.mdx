...
<Box color='bw'>
 <font size="+2"> **Hands-on exercise:**</font>
...
<p style={{ color: "purple"}}>  Finding a maximum dt based on the CFL stability constraint
Suppose we have a flat plate turbulent boundary layer with the known characteristics:</p>
- <p style={{ color: "purple"}}>Friction velocity: $u_\tau=0.5$ m/s</p>
- <p style={{ color: "purple"}}>Viscosity $\nu=1.8205 \times 10^{-5}$ kg/(m*s)</p>

<p style={{ color: "purple"}}>Using the 1/7th power law to approximate the turbulent boundary layer ($\bar{u}/u_\tau=8.7(y u_\tau/\nu)^{1/7}$), compute the CFL number if you set the time advancement to $dt=1 \times 10^{-5}$ s and that your first grid point satisfies $y^+=1$ and your grid has an expansion ratio of 1.025.</p>

<p style={{ color: "purple"}}> **2.3.1 How much walltime do you need?**</p>
<MultipleChoice>
    <Option>
TEST
    </Option>
...
<details>
    <summary>2.3.1 Solution </summary>
...
<p style={{ color: "purple"}}>**2.3.1 Suppose that you want to simulate a case at a $Re_ \delta=10,000$ estimate the grid requirement. (need to refine example)**</p>
...
<details>
    <summary><p style={{ color: "gray"}}>2.3.2 Solution</p></summary>
