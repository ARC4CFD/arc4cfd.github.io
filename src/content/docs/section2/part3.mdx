...
Although the spatial discretization is likely the largest contributor to the computational cost, the time advancement can also have a significant impact. Starting from the intial state, the governing equations will be advanced in time (assuming an unsteady simulation) by a time-step, $dt$, that may be constant or varying. Naturally, the allowable $dt$ will constrained by  either:

- stability of the temporal scheme of the code (numerics);
...
:::caution[Caution]
This estimate does not consider the viscous stability. 
:::
...
To estimate the overall cost of the numerical simulation one must consider that the equations of motion must be integrated for a time proportional to the integral time-scale of the flow $T$. The time step size $\Delta t$ is a function of the simulation technique used. In DNS, for instance, $\Delta t$ is limited by the need of resolving the life cycle of the smallest eddy of size $\eta$.
