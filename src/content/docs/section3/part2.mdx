...
The majority of the research data is of modest size (Mb rage), but the raw CFD results can be significantly larger (Gb or Tb range), especially for unsteady problem, where multiple snapshots may need to be saved. Therefore, most of the work in data management for CFD lies in the organization of the data with the supplementary challenge of curating and storing of large data sets.  



### Do we need to store all results?
Post-processed data (averaged flow fields, point statistics etc.) should be curated and organized, as these represent important research outcomes that have likely made their way to the final publication.

Simulated results generated from CFD are inherently deterministic (assuming there is no purely stochastic events in the simulation). In other words, given the identical initial state, boundary conditions, and solver characteristics, we should be able to reproduce the same dataset to within numerical precision. In this context, the first question we need to ask is: do we need to store all the data?
...
Given this, it is often more advantageous to:
- Active storage of lots of **hot data** (still under active use) HPC systems: allows recalculation of statistics, post-processing etc.

As the data becomes **cold** (no longer under active use), then:
- Archive some of the simulation data to disk (external hard drive, $25-$50/Tb): difficult to share but can be retrieved if needed
- Curate a smaller subset to share (more expensive database)
...
- ability to run code same CFD code on future HPC systems.
