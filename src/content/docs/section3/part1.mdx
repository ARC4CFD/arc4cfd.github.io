...
#### Local data storage
Local data storage is typically done at an individual or small research group level. It includes data storage on local hard-drive/SSD directly on a workstation, external hard-drive/USB drive, network attached storage (NAS). These local storage modalities have the advantage of being physically accessible and, in the case of external hard-drives, can be a cost effective way to archive or backup data.
...
##### HPC data storage 
 Digital Research Alliance of Canada provides data storage facilities to meet the users' storage needs. Details on the data storage facilities are listed [here](https://alliancecan.ca/en/services/advanced-research-computing/national-services/storage).
